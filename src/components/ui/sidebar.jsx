...
const Sidebar = React.forwardRef(function Sidebar(
  ...
  if (collapsible === "none") {
    return (
      <div ref={ref} className={cn("flex h-full w-64 flex-col bg-sidebar", className)} {...props}>
        {children}
      </div>
    );
  }
  ...
  if (isMobile) {
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile}>
        <SheetContent side={side} className="w-[18rem] p-0 bg-sidebar">
  ...
  return (
    <div
      ...
      data-side={side}
    >
      <div className="fixed inset-y-0 w-64 bg-sidebar">
        <div className="flex h-full flex-col">{children}</div>
      </div>
    </div>
  );
